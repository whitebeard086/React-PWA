...
import { GetUsersResponse, UserWithBookings } from '../../utils/types';
...
type Props = {
    usersData: Partial<GetUsersResponse>
    loading: boolean
}
...
const ActionColumn = ({ row }: { row: UserWithBookings }) => {
    ...
    const onView = (user: UserWithBookings) => {
        navigate(`/users/clients/${user.slug}`)
    }
...
const ClientColumn = ({ row }: { row: UserWithBookings }) => {
...
const AllClients = ({ usersData, loading }: Props) => {
    const { allClients: data } = usersData
    ...
    const columns = useMemo<ColumnDef<UserWithBookings>[]>(
        () => [
            { 
                header: 'Full Name', 
