		...
		<div>
						...
						<div className="flex items-center gap-4 justify-between mt-2">
							<div className="flex items-center gap-1">
								...
								<p className="text-gray-500">
									({service?.bookings?.length?.toLocaleString()})
								</p>
							</div>

							<p className="text-gray-300 font-semibold">
								{millify(service?.bookings?.length)}{' '}
								{service?.bookings?.length === 1 ? 'Order' : 'Orders'}
							</p>
						</div>
					</Card>
				</div>
			...
			<Card className="mt-14">
				<div className="flex flex-col gap-4">
					...
					<div>
						<h4 className="font-bold text-base mb-2">Working Hours</h4>
			...
			{signedIn ? (
				...
				<div className="mt-4">
					<Link to="/login" state={{ provider_id: provider?.id }}>
						<Button variant="solid" block icon={<AiFillWechat />}>
							Login to Chat
						</Button>
					</Link>
