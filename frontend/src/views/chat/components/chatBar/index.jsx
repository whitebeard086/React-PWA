...
const ChatBar = ({ onCreateInvoice }) => {
    ...
    const { provider, invoice, booked } = useSelector(
    ...
    return (
        <div className="p-4 flex items-center gap-4 justify-between bg-white border-t-2 sticky top-[4.7rem] z-10">
            <div className="flex gap-1 items-center">
                <div
                    onClick={() => navigate(-1)}
                    className="hover:bg-emerald-50 transition duration-300 h-10 w-10 flex items-center justify-center rounded-full cursor-pointer"
                >
                    <HiArrowNarrowLeft className="text-2xl" />
                </div>
                <Link
                    to={`/browse/profile/${provider?.username}`}
                    className="flex gap-2 items-center"
                >
                    <Avatar
                        src={`${imagePath}/${
                            provider?.service?.banner || provider?.image
                        }`}
                        size="md"
                        shape="circle"
                    />
                    <div className="flex flex-col">
                        <h4 className="font-bold text-base">
                            {provider?.service?.title || provider?.username}
                        </h4>
