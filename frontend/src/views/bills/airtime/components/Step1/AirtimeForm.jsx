										...
										<FormContainer>
													...
													{({ field, form }) => {
														return (
															<FormNumericInput
																...
																isAllowed={(field) =>
																	field.value <=
																	Number(airtime?.meta?.maximum_fee)
																}
																placeholder={`Amount between ₦${Number(
																	airtime?.meta?.minimum_fee
																).toLocaleString()} - ₦${Number(
																	airtime?.meta?.maximum_fee
																).toLocaleString()}`}
																decimalScale={0}
																value={field.value}
																inputPrefix={
											...
											<Button
												...
												disabled={
													!values.amount ||
													!values.phone ||
													values.amount < Number(airtime?.meta?.minimum_fee) ||
													values.amount > Number(airtime?.meta?.maximum_fee)
												}
											>
												Continue
											</Button>
