...
import { getDashboardData, openDispute, setDisputeStatusDash } from '../store/dataSlice';
...
const DisputeDialog = () => {
    ...
    const { openingDispute, disputeStatus, booking, dispute } = useSelector((state) => state.dashboard.data)
    const { openDisputeDialog, bookingID } = useSelector((state) => state.dashboard.state)
    const { profile } = useSelector((state) => state.auth.user);
    const { createStatus, notification } = useSelector((state) => state.notifications.data);

    const receiver = booking?.user
    ...
    useEffect(() => {
        if (disputeStatus === 'error') {
            ...
            dispatch(setDisputeStatusDash('idle'))
        }
